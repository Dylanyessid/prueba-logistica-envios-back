...
export default {
  ...
  async updateSeaShipment(req: Request, res: Response) {
    try {
      const id = Number(req.params.id);
      const result = await seaShipmentService.updateSeaShipment(id, req.body);
      ...
      return res.status(200).json({
        success: true,
        message: "Sea shipment updated successfully",
        data: result.value,
      });
    } catch (error) {
      handleHttpError(res, error);
